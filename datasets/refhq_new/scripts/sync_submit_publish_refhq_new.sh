...
ssh -S "${SOCK}" -o BatchMode=yes "${HOST}" \
  "mkdir -p ${STAGING}/datasets/refhq_new/scripts ${STAGING}/datasets ${RUN_DIR}/logs"
...
STAGING=${STAGING}
RUN_DIR=${RUN_DIR}
mkdir -p "\${RUN_DIR}/datasets/refhq_new/scripts"
...
FIN_JOB=\$(sbatch --parsable --exclude=wheat-01 \
  --chdir="\${RUN_DIR}" \
  --export=ALL,RUN_DIR="\${RUN_DIR}",VENV="\${VENV}",PLAN="\${PLAN}",STAGE_DIR="\${STAGE_DIR}",SCRATCH_ROOT="\${RUN_DIR}",REFHQ_NEW_SCRIPTS="\${REFHQ_NEW_SCRIPTS}" \
