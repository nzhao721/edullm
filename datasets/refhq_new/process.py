...
DEFAULT_ENGLISH_DOMAIN = "english"
DEFAULT_ENGLISH_SCORE_THRESHOLD = 0.5
...
def render_taggers_config(
    *,
    domain: str = DEFAULT_ENGLISH_DOMAIN,
    documents_glob: str,
    work_dir: Path,
    processes: int,
...
def render_pre_mix_config(
    *,
    ...
    work_dir: Path,
    processes: int,
    english_score_threshold: float = DEFAULT_ENGLISH_SCORE_THRESHOLD,
    config_root: Path = DOLMA_CONFIG_ROOT,
    ...
    env = {
        "WORK_DIR": work_dir.as_posix(),
        "DOLMA_PROCESSES": str(processes),
        "ENGLISH_SCORE_THRESHOLD": str(english_score_threshold),
    }
...
def _run_dolma(config_path: Path, command: str) -> None:
    ...
    # nltk's import-security finder blocks `defusedxml` when cwd is on some
    # NFS scratch trees (even with PYTHONSAFEPATH). Neutral cwd avoids that.
    subprocess.run(
        [executable, "--config", str(config_path), command],
        check=True,
        cwd="/tmp",
    )
...
def apply_dolma_english_filter(
    *,
    shard_path: Path,
    work_root: Path,
    processes: int = 1,
    ...
    """Tag and English-mix one local document shard in place using Dolma configs."""
    ...
    logger.info("%s: dolma english pre-mix start", domain)
    ...
    logger.info("%s: dolma english pre-mix wrote %d documents", domain, document_count)
    return document_count


# Alias matching RefHQ naming for callers that expect apply_dolma_pre_mix_domain.
def apply_dolma_pre_mix_domain(
    *,
    ...
    return apply_dolma_english_filter(
