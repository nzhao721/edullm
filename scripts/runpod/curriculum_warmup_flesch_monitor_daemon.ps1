# 5m poller for curriculum warmup-flesch on RunPod. Emits AGENT_LOOP_TICK lines for Cursor.
    ...
    try {
        ...
        $json = @{
            prompt = if ($status.complete) {
                "Curriculum warmup-flesch training is complete on pod $podId (step 2384 eval done). Summarize final metrics from $trainLog and W&B project curriculum. Do NOT terminate the pod unless the user asks."
            } else {
