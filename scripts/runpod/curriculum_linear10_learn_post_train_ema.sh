#!/usr/bin/env bash
# Post-training EMA merge + task-loss eval for curriculum linear10-learn.
set -Eeuo pipefail

REPO_DIR="${REPO_DIR:-/workspace/OLMo-core}"
RUN_DIR="${RUN_DIR:-/workspace/edullm-runs/curriculum/linear10-learn}"
ARM="${ARM:-linear10-learn}"
WANDB_ENV_FILE="${WANDB_ENV_FILE:-/workspace/wandb-session.env}"
EMA_LOG="${EMA_LOG:-/workspace/curriculum-linear10-learn-ema.log}"
...
log "starting post-hoc EMA for ${ARM} (local eval only; W&B upload deferred to clean replay)"
