#!/usr/bin/env bash
# Post-training EMA merge + task-loss eval for curriculum warmup-flesch.
# Logs to the same W&B run as training (step 2385 eval point / final model).
set -Eeuo pipefail

REPO_DIR="${REPO_DIR:-/workspace/OLMo-core}"
RUN_DIR="${RUN_DIR:-/workspace/edullm-runs/curriculum/warmup-flesch}"
ARM="${ARM:-warmup-flesch}"
WANDB_ENV_FILE="${WANDB_ENV_FILE:-/workspace/wandb-session.env}"
EMA_LOG="${EMA_LOG:-/workspace/curriculum-warmup-flesch-ema.log}"
