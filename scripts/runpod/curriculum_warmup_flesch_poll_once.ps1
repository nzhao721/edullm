# One-shot curriculum warmup-flesch status poll (training + post-hoc EMA).
$ErrorActionPreference = "Continue"
$lock = Join-Path $env:TEMP "curriculum-warmup-flesch-monitor-tick.lock"
if (Test-Path $lock) {
...
$trainLog = "/workspace/curriculum-warmup-flesch-train.log"
$emaLog = "/workspace/curriculum-warmup-flesch-ema.log"
$runDir = "/workspace/edullm-runs/curriculum/warmup-flesch"
...
function Write-Tick {
    param([string]$Body)
    $line = "AGENT_LOOP_TICK_CURRICULUM_WARMUP_FLESCH $Body"
...
try {
    ...
    $json = @{
        prompt = if ($status.complete) {
            "Curriculum warmup-flesch training and EMA eval are complete on pod $podId. Summarize final step-2384 + EMA step-2385 metrics from W&B project curriculum. Do NOT terminate the pod unless the user asks."
        } elseif ($status.training_complete -and $status.ema_running) {
            "Curriculum warmup-flesch training is complete on pod $podId. Post-hoc EMA + step-2385 task-loss eval is running. Report EMA progress from $emaLog."
        } elseif ($status.training_complete -and -not $status.ema_complete) {
            "Curriculum warmup-flesch training is complete on pod $podId. Post-hoc EMA should be running or pending."
        } else {
            "Check curriculum warmup-flesch RunPod training on pod $podId. Fetch $trainLog, verify 8-GPU trainer is running, and reply with a concise progress + ETA summary for the user."
        }
    }
    ...
    Write-Tick (@{
        prompt = "Check curriculum warmup-flesch RunPod training progress on pod $podId."
