#!/usr/bin/env bash
# Post-training EMA merge + task-loss eval for curriculum linear10-mtld.
# Logs to the same W&B run as training (step 2385 eval point / final model).
set -Eeuo pipefail

REPO_DIR="${REPO_DIR:-/workspace/OLMo-core-curriculum-mtld}"
RUN_DIR="${RUN_DIR:-/workspace/edullm-runs/curriculum/linear10-mtld}"
ARM="${ARM:-linear10-mtld}"
WANDB_ENV_FILE="${WANDB_ENV_FILE:-/workspace/wandb-session-curriculum.env}"
EMA_LOG="${EMA_LOG:-/workspace/curriculum-linear10-mtld-ema.log}"
