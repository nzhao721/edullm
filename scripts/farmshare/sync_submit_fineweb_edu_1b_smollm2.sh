#!/usr/bin/env bash
# Sync tokenization scripts to FarmShare scratch and submit the 1B-token FineWeb-Edu job.
set -Eeuo pipefail
...
rsync -avz -e "ssh -S ${SOCK} -o BatchMode=yes" \
  ...
  "${SRC}/scripts/farmshare/submit_fineweb_edu_1b_smollm2.sh" \
...
ssh -S "${SOCK}" -o BatchMode=yes "${HOST}" bash "${STAGING}/scripts/farmshare/submit_fineweb_edu_1b_smollm2.sh"
