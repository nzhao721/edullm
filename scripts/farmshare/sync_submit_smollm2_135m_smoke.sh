#!/usr/bin/env bash
# Sync SmolLM2 smoke-train scripts to FarmShare and submit a 1-GPU job.
set -Eeuo pipefail
...
ssh -S "${SOCK}" -o BatchMode=yes "${HOST}" bash "${STAGING}/scripts/farmshare/submit_smollm2_135m_smoke.sh"
