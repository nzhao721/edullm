...
def install_dtensor_stubs() -> None:
    ...
    # Ensure newer DTensor import path resolves.
...
def to_local(t: Any) -> torch.Tensor:
    # Prefer raw local storage first: DeviceMesh may be incompletely unpickled,
    # so full_tensor()/to_local() can fail even when _local_tensor is intact.
