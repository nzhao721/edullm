...
def test_build_A_uses_uniform_minus_onehot_and_clips_negative_values():
    uniform = {family: 1.0 + j for j, family in enumerate(CURVE_FAMILIES)}
    runs = [_run("probe_uni", uniform)]
    for i, domain in enumerate(DOMAINS):
        losses = dict(uniform)
        losses[CURVE_FAMILIES[0]] = uniform[CURVE_FAMILIES[0]] - (0.1 * (i + 1))
        losses[CURVE_FAMILIES[1]] = uniform[CURVE_FAMILIES[1]] + 0.5
        runs.append(_run(f"probe_{domain}", losses))

    A, detail = build_A_from_extrapolated({"chinchilla_steps": 5806, "runs": runs})
