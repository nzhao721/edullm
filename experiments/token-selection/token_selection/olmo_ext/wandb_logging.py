...
* Log train metrics, task-loss eval metrics (+ artifact), and complete
  checkpoint directories (+ model artifact).

S3 is input-only: it may stage published training data and bootstrap references
at run start, but is never used for checkpoints, progress, evals, or resume.
"""
...
PRODUCTION_MAX_TOKENS = 9_900_000_000
PRODUCTION_TOTAL_STEPS = 2360
...
def is_production_run(*, max_tokens: int, total_steps: int) -> bool:
    """Return whether this is the locked 9.9B/2360-step production budget."""
    return (
        int(max_tokens) == PRODUCTION_MAX_TOKENS
        and int(total_steps) == PRODUCTION_TOTAL_STEPS
    )
...
def require_wandb_for_production(
    ...
    *,
    ...
    """Fail before training when a production online run cannot upload artifacts."""
    if production_online(production=production, mode=mode) and run is None:
        raise WandbArtifactError(
            "production online runs require an initialized W&B run and WANDB_API_KEY; "
...
def _artifact_slug(value: str) -> str:
    ...
    return slug or "token-selection"


def checkpoint_artifact_name(run_name: str) -> str:
    """Stable per-run artifact collection; each checkpoint is a new version."""
...
def checkpoint_artifact_ref(
    *,
    ...
    collection = checkpoint_artifact_name(run_name)
    prefix = f"{entity}/" if entity else ""
    return f"{prefix}{project}/{collection}:{alias}"
...
def add_wandb_argparse_options(
    ...
    *,
    ...
    parser.add_argument(
        ...
        help=(
            "W&B model artifact ref to restore into runtime scratch before resume "
            "(for example entity/token-selection/run-checkpoint:latest)"
        ),
    )
...
def _task_loss_label_values(payload: Mapping[str, Any]) -> dict[str, float]:
    """Merge current ``labels`` and legacy ``task_loss_bpb`` numeric mappings."""
...
def task_loss_payload_complete(payload: Mapping[str, Any]) -> bool:
    """True only when all 20 raw BPB labels are present and numeric."""
    values = _task_loss_label_values(payload)
    return _TASK_LOSS_RAW_LABEL_SET.issubset(values)


def task_loss_metrics(payload: Mapping[str, Any]) -> dict[str, float]:
    """Flatten current/legacy task-loss JSON without blessing partial suites."""
    metrics: dict[str, float] = {}
    labels = _task_loss_label_values(payload)
    if task_loss_payload_complete(payload):
        # Recompute instead of trusting historical macro fields, some of which
        # incorrectly included derived MMLU/core aggregates.
...
def wandb_log_eval(
    ...
    *,
    ...
    if run is None:
        if strict:
            raise WandbArtifactError("required W&B eval artifact has no active run")
        return
    ...
    try:
        logged = run.log_artifact(art, aliases=[f"step-{int(step):07d}"])
        if strict and hasattr(logged, "wait"):
            logged.wait()
    except Exception as exc:  # noqa: BLE001
        if strict:
            raise WandbArtifactError(
                f"required W&B eval artifact upload failed for step {int(step)}"
            ) from exc
        log.warning("wandb eval artifact upload failed for step %s: %s", step, exc)
...
def wandb_log_checkpoint(
    ...
    *,
    ...
    if not upload_artifact or _wandb is None:
        if strict:
            raise WandbArtifactError(
                "required W&B checkpoint upload is disabled or wandb is unavailable"
            )
        return None
    path = Path(ckpt_dir)
    if not path.exists():
        if strict:
            raise WandbArtifactError(f"checkpoint artifact source does not exist: {path}")
        return None
    meta = {"step": int(step)}
    if tokens_seen is not None:
    ...
    art = _wandb.Artifact(
        name=checkpoint_artifact_name(
            run_name or str(getattr(run, "name", "") or "token-selection")
        ),
        type="model",
        metadata=meta,
    )
    # Upload the complete step directory. Optimizer, trainer/callback, EMA, and
    # fingerprint state are all required for a scientifically valid resume.
    art.add_dir(str(path))
    aliases = ["latest", f"step-{int(step):07d}"]
    try:
        logged = run.log_artifact(art, aliases=aliases)
        if strict and hasattr(logged, "wait"):
            logged.wait()
    except Exception as exc:  # noqa: BLE001
        if strict:
        ...
        log.warning("wandb checkpoint upload failed for step %s: %s", step, exc)
        return None
    log.info("wandb uploaded checkpoint artifact %s aliases=%s", art.name, aliases)
    return logged
...
def wandb_log_directory_artifact(
    ...
    *,
    ...
    """Upload a local progress/eval directory without any S3 side channel."""
    source = Path(path)
    if run is None or _wandb is None:
        if strict:
            raise WandbArtifactError(f"required W&B {artifact_type} artifact has no active run")
        return None
    if not source.exists():
        if strict:
            raise WandbArtifactError(f"artifact source does not exist: {source}")
        return None
    artifact = _wandb.Artifact(name=_artifact_slug(name), type=artifact_type)
    if source.is_dir():
    ...
    try:
        ...
        log.warning("wandb %s artifact upload failed: %s", artifact_type, exc)
        return None
...
def restore_checkpoint_artifact(
    ...
    *,
    ...
    """Download one W&B checkpoint artifact into local runtime scratch.

    The artifact metadata determines the ``stepN`` destination. The embedded
    schema-v2 fingerprint is also copied to the save-folder root so the existing
    scientific resume guard remains authoritative.
    """
    if _wandb is None and api is None:
        raise WandbArtifactError("wandb package is required to restore a checkpoint")
    ref = str(artifact_ref).strip()
    if not ref:
        raise WandbArtifactError("a non-empty W&B checkpoint artifact reference is required")
    root = Path(save_folder)
    root.mkdir(parents=True, exist_ok=True)
    client = api or _wandb.Api()
    try:
        artifact = client.artifact(ref, type="model")
        metadata = dict(getattr(artifact, "metadata", {}) or {})
        step = int(metadata["step"])
        with tempfile.TemporaryDirectory(prefix=".wandb-restore-", dir=str(root)) as tmp:
            downloaded = Path(artifact.download(root=tmp))
            target = root / f"step{step}"
            if target.exists():
                shutil.rmtree(target)
            shutil.copytree(downloaded, target)
    except Exception as exc:  # noqa: BLE001
        raise WandbArtifactError(f"failed to restore W&B checkpoint artifact {ref!r}") from exc
    fingerprint = target / "run_fingerprint.json"
    if require_fingerprint and not fingerprint.is_file():
        raise WandbArtifactError(
            f"W&B checkpoint artifact {ref!r} is missing run_fingerprint.json"
        )
    if fingerprint.is_file():
        shutil.copy2(fingerprint, root / "run_fingerprint.json")
    log.info("restored W&B checkpoint %s → %s", ref, target)
    return target
    ...
    def poll(self) -> list[int]:
        ...
        for path in sorted(self.results_dir.glob("step*_task_loss.json")):
            ...
            if not isinstance(payload, Mapping):
                continue
            # A valid partial JSON can be overwritten by the evaluator later.
            # Do not mark/log it as a completed 20-label scientific suite.
...
def wandb_run_from_trainer(trainer: Any) -> Any | None:
    """Best-effort extract of the active W&B run from olmo-core."""
...
class WandbArtifactsCallback:  # subclassed onto olmo Callback when available
    ...
    def _maybe_log_checkpoint(self, step: int) -> None:
        ...
        run = self.poller.run
        strict_upload = (
            self.upload_checkpoint_artifacts
            and production_online(
                production=self.production, mode=self.wandb_mode
            )
        )
        if run is None:
            if strict_upload:
                raise WandbArtifactError(
                    f"required W&B checkpoint upload has no active run at step {step}"
                )
            return
        ...
        if not step_dir.exists():
            if strict_upload:
                raise WandbArtifactError(
                    f"required checkpoint artifact source does not exist: {step_dir}"
                )
            return
    ...
    def post_train(self) -> None:  # pragma: no cover
        ...
        if self.progress_dir is not None:
            wandb_log_directory_artifact(
                self.poller.run,
                self.progress_dir,
                name=f"{self.run_name or 'token-selection'}-progress",
                artifact_type="metrics",
                strict=production_online(
                    production=self.production, mode=self.wandb_mode
                ),
            )
